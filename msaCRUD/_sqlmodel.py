...
from .base import MSABaseCrud
from .base_model import MSABaseModel
from .parser import (MSASQLModelFieldParser, SQLModelField, SQLModelListField,
                     get_python_type_parse)
from .schema import MSACRUDListSchema, MSACRUDOut
from .utils import (parser_item_id, parser_str_set_list,
...
class MSASQLModelSelector:
    ...
    def __init__(
        self, model: Type[SQLModel] = None, fields: List[SQLModelListField] = None
    ) -> None:
        ...
        self.parser = MSASQLModelFieldParser(self.model)
    ...
    async def get_select(self, request: Request) -> Select:
        return (
            select(*self._list_fields_ins.values())
            if self._list_fields_ins
    ...
    def _calc_ordering(self, orderBy, orderDir):
        ...
        if insfield:
            ...
            order = self.parser.filter_insfield(
                self.ordering, save_class=(UnaryExpression,)
            )
        return order
        ...
        for k, v in data.items():
            ...
            if insfield:
                operator, val = self._parser_query_value(
                    v, python_type_parse=get_python_type_parse(insfield)
                )
                if operator:
...
class MSASQLModelCrud(MSABaseCrud, MSASQLModelSelector):
    ...
    def __init__(
        ...
        MSASQLModelSelector.__init__(self, model, fields)
        MSABaseCrud.__init__(self, self.model, router)
    ...
    def _create_schema_update(self):
        ...
        if not self.readonly_fields and not self.update_fields:
            return super(MSASQLModelCrud, self)._create_schema_update()
        self.update_fields = (
            self.parser.filter_insfield(self.update_fields)
            or self.schema_model.__fields__.values()
        )
        modelfields = {
            self.parser.get_modelfield(ins, deepcopy=True) for ins in self.update_fields
        }
        modelfields = {
            field
        ...
        return schema_create_by_modelfield(
            f"{self.schema_name_prefix}Update", modelfields, set_none=True
        )
    ...
    def _create_schema_create(self):
        ...
        if not self.create_fields:
            return super(MSASQLModelCrud, self)._create_schema_create()
        modelfields = list(
            filter(
                None,
                [
        ...
        return schema_create_by_modelfield(
            f"{self.schema_name_prefix}Create", modelfields
        )
    ...
    def _read_items(self, session: Session, item_id: List[str]):
        ...
        parse = (
            ...
            if self.schema_read.Config.orm_mode
    ...
    def schema_name_prefix(self):
        if self.__class__ is MSASQLModelCrud:
            return self.model.__name__
        return super().schema_name_prefix

    async def on_create_pre(
        self, request: Request, obj: MSABaseModel, **kwargs
    ) -> Dict[str, Any]:
    ...
    def route_list(self) -> Callable:
        async def route(
            ...
            data.items = await self.db.async_execute(
                stmt, on_close_pre=lambda r: r.all()
            )
            data.items = self.parser.conv_row_to_dict(data.items)
            data.items = (
                [self.schema_list.parse_obj(item) for item in data.items]
                if data.items
                else []
            )
    ...
    def route_create(self) -> Callable:
        async def route(
        ...
        ) -> MSACRUDOut[Union[int, self.schema_model]]:  # type: ignore
            ...
            if is_bulk:
                return MSACRUDOut(data=getattr(result, "rowcount", None))
            data = values[0]
